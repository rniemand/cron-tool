...
[TestFixture]
public class GreaterThanOrEqualComparatorTests
{
  [Test]
  public void Compare_GivenCalled_ShouldLog()
  {
    // arrange
    var logger = Substitute.For<ILoggerAdapter<GreaterThanOrEqualComparator>>();
  ...
  public void Compare_GivenConstructed_ShouldReturn_Comparator() =>
    Assert.That(GetComparator().Comparator, Is.EqualTo(Comparator.GreaterThanOrEqual));

  private static GreaterThanOrEqualComparator GetComparator(ILoggerAdapter<GreaterThanOrEqualComparator> logger = null) =>
    new(logger ?? Substitute.For<ILoggerAdapter<GreaterThanOrEqualComparator>>());
}
