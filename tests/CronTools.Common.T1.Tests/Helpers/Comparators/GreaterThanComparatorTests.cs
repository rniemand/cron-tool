...
public class GreaterThanComparatorTests
{
  ...
  {
    // arrange
    var logger = Substitute.For<ILoggerAdapter<GreaterThanComparator>>();
  ...
  public void Compare_GivenConstructed_ShouldReturn_Comparator() =>
    Assert.That(GetComparator().Comparator, Is.EqualTo(Comparator.GreaterThan));

  private static GreaterThanComparator GetComparator(ILoggerAdapter<GreaterThanComparator> logger = null) =>
    new(logger ?? Substitute.For<ILoggerAdapter<GreaterThanComparator>>());
}
