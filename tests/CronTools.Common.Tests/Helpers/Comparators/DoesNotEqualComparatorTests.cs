...
[TestFixture]
public class DoesNotEqualComparatorTests
{
  [Test]
  public void Compare_GivenCalled_ShouldLog()
  {
    // arrange
    var logger = Substitute.For<ILoggerAdapter<DoesNotEqualComparator>>();
  ...
  public void Compare_GivenConstructed_ShouldReturn_Comparator() =>
    Assert.That(GetComparator().Comparator, Is.EqualTo(Comparator.DoesNotEqual));

  private static DoesNotEqualComparator GetComparator(ILoggerAdapter<DoesNotEqualComparator> logger = null) =>
    new(logger ?? Substitute.For<ILoggerAdapter<DoesNotEqualComparator>>());
}
